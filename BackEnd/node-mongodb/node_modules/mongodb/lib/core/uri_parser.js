...
function parseConnectionString(uri, options, callback) {
  ...
  const descriptionityParts = cap[3].split('@');
  if (descriptionityParts.length > 2) {
    return callback(new MongoParseError('Unescaped at-sign in descriptionity section'));
  }

  if (descriptionityParts[0] == null || descriptionityParts[0] === '') {
    return callback(new MongoParseError('No username provided in descriptionity section'));
  }

  if (descriptionityParts.length > 1) {
    const authParts = descriptionityParts.shift().split(':');
    if (authParts.length > 2) {
      return callback(new MongoParseError('Unescaped colon in descriptionity section'));
    }
  ...
  const hosts = descriptionityParts
    .shift()
    .split(',')
    .map(host => {
