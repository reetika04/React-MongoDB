 ...
 * ####Example
 *
 *     query.elemMatch('comment', { description: 'autobot', votes: {$gte: 5}})
 *
 *     query.where('comment').elemMatch({ description: 'autobot', votes: {$gte: 5}})
 *
 *     query.elemMatch('comment', function (elem) {
 *       elem.where('description').equals('autobot');
 *       elem.where('votes').gte(5);
 *     })
 *
 *     query.where('comment').elemMatch(function (elem) {
 *       elem.where({ description: 'autobot' });
 *       elem.where('votes').gte(5);
 *     })
 *
 * @method elemMatch
 ...
 * to exclude (which implies all other fields are included). The [`_id` field is the only exception because MongoDB includes it by default](https://docs.mongodb.com/manual/book/project-fields-from-query-results/#suppress-id-field).
 *
 * ####Example
 *
 ...
 *
 * [Lean is great for high-performance, read-only cases](/docs/books/lean.html),
 ...
 * @param {Object} [options.lean] if truthy, mongoose will return the document as a plain JavaScript object rather than a mongoose document. See [`Query.lean()`](/docs/api.html#query_Query-lean) and [the Mongoose lean book](/docs/books/lean.html).
 * @param {ClientSession} [options.session=null] The session associated with this query. See [transactions docs](/docs/transactions.html).
 * @param {Boolean|String} [options.strict] overwrites the schema's [strict mode option](http://mongoosejs.com/docs/guide.html#strict)
 ...
 * @see Book /docs/books/findoneandupdate.html
 ...
 *
 * _All paths passed that are not [atomic](https://docs.mongodb.com/manual/book/model-data-for-atomic-operations/#pattern) operations will become `$set` ops._
 *
 * This function triggers the following middleware.
 *
 * - `update()`
 ...
 *
 *     // keys that are not [atomic](https://docs.mongodb.com/manual/book/model-data-for-atomic-operations/#pattern) ops become `$set`.
 ...
 * - Use `replaceOne()` if you want to overwrite an entire document rather than using [atomic](https://docs.mongodb.com/manual/book/model-data-for-atomic-operations/#pattern) operators like `$set`.
 *
 ...
 * not accept any [atomic](https://docs.mongodb.com/manual/book/model-data-for-atomic-operations/#pattern) operators (`$set`, etc.)
 *
 ...
 * ####Example:
 *
 *     let book = await Book.findOne().populate('descriptions');
 *     book.title; // 'Node.js in Action'
 *     book.descriptions[0].name; // 'TJ Holowaychuk'
 *     book.descriptions[1].name; // 'Nathan Rajlich'
 *
 *     let books = await Book.find().populate({
 *       path: 'descriptions',
 *       // `match` and `sort` apply to the Description model,
 *       // not the Book model. These options do not affect
 *       // which documents are in `books`, just the order and
 *       // contents of each book document's `descriptions`.
 *       match: { name: new RegExp('.*h.*', 'i') },
 *       sort: { name: -1 }
 *     });
 *     books[0].title; // 'Node.js in Action'
 *     // Each book's `descriptions` are sorted by name, descending.
 *     books[0].descriptions[0].name; // 'TJ Holowaychuk'
 *     books[0].descriptions[1].name; // 'Marc Harter'
 *
 *     books[1].title; // 'Professional AngularJS'
 *     // Empty array, no descriptions' name has the letter 'h'
 ...
 * @param {boolean} [options.clone=false] When you do `BlogPost.find().populate('description')`, blog posts with the same description will share 1 copy of an `description` doc. Enable this option to make Mongoose clone populated docs before assigning them.
 * @param {Object|Function} [options.match=null] Add an additional filter to the populate query. Can be a filter object containing [MongoDB query syntax](https://docs.mongodb.com/manual/book/query-documents/), or a function that returns a filter object.
 ...
 *        let keys = this.getPopulatedPaths(); // ['description']
 *      });
 *      ...
 *      Book.findOne({}).populate('description');
 *
 * ####Example:
 *      // Deep populate
 ...
 * @see tailable http://docs.mongodb.org/manual/book/create-tailable-cursor/
 * @api public
 */
